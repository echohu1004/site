...
var IFlyIatSession = (function (window, navigator) {
    ...
    var ajaxReq=function (path,fd,fun,errfun) {
        var url = path;//接口
        url = url ? url : '';
        var XHR = null;
    ...
    var newSocket = function (url) {
        ...
        var serverMessageHandler = (function () {
            ...
            return {
                ...
                /**
                 * 请求结果响应处理
                 * @param obj e.g. {"ver" : "1.0", "sub" : "iat", "cmd" : "grs", "msg" : "response","ret":0, "data" : { "sid" : "iat@..","iatrst":"识别结果"}}
                 */
                "onGRS": function (obj) {
                    if (rec_state != recorderStatus.audioWrite) {//on status error
                ...
                "onSSE": function (obj) {
                    sessionInfo.id = null;
                    console.log("iat websokcet cmd==end")
        ...
        var onclose = function () {
        	sessionInfo.id = null;
            callback.onProcess('onStop');
        	console.log("iat websocket onClose is ok");
        };
        var onopen = function () {
            console.log("iat websokcet onOpen is ok");
        };
...
//            }
	...
	var inputSampleRate  = 44000;  //输入采样率	
	var outputSampleRate = 16000.0;
    ...
    var audioSender = (function () {
        ...
        var addBuffer = function (data) {
	     ...
	     var downsample = function(e) {
            var t, n, r = null, i = e.length,o = bufferUnusedSamples.length;
            if (o > 0) {
                for (r = new Float32Array(o + i), t = 0; t < o; ++t){
					r[t] = bufferUnusedSamples[t];
				} 
            ...
            //var a = this.options.sourceSampleRate / 16e3;
            var a = inputSampleRate / outputSampleRate;
            var u = Math.floor((r.length - s.length) / a) + 1;
            ...
            var h = Math.round(a * t),
                d = r.length - h;
            if (d > 0){
                for (bufferUnusedSamples = new Float32Array(d), t = 0; t < d; ++t){
					bufferUnusedSamples[t] = r[h + t];
				} 
            } else {
	     ...
	     // pcm编码
	     var encodePCM = function () {
	         //let bytes = compress(),
	    	var output = inputData.splice(0, inputData.length);
            //合并
            var bytes = new Float32Array(size);
            size = 0; //计数清空
            frameCount = 0;//计数清空
            var offset = 0;
            for (var i = 0; i < output.length; i++) {
            	bytes.set(output[i], offset);
                offset += output[i].length;
            }
	         //let bytes = inputData.splice(0, inputData.length),
	     ...
	     var encodeWAV = function () {
            ...
            for (var i = 0; i < output.length; i++) {
	         ...
	         var sampleBits = outputSampleRate; //Math.min(inputSampleBits, oututSampleBits);
        ...
        var sendData_Blob_PCM = function () {
            var pcmBlob = new Blob([encodePCM()], { type: 'audio/pcm' });
            command.audioWrite(sessionInfo.id,pcmBlob);
        };
        return {
            "sendData": function (data) {
                if (!checkStatus()) return; // 验证是否存在sessionid
                rec_state = recorderStatus.audioWrite;
                if (frameCount < 5) {//缓冲6帧再发送
                    addBuffer(data);
                    frameCount++;
                    return;
                } 
    ...
    var gotStream = function (stream) {
        ...
        audioNode.scriptNode.onaudioprocess = function (e) {
            ...
            audioSender.sendData(e.inputBuffer.getChannelData(0));
        };
    ...
    return function (setting) {
        ...
        (function () {
            ...
            env.hostContext=window.location.protocol+"//"+window.location.host+"/";
        })();
       ...
       // socket = newSocket(settings.serverUrl);
        this.start = function (iat_params_obj) {
            ...
            settings.serverUrl=iat_params_obj.iat_server;
            socket = newSocket(settings.serverUrl);
            iatEvent.startRecord();
        };
        console.log("guid",utils.guid());
        /*
